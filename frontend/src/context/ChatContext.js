...
export const ChatProvider = ({ children }) => {
  ...
  const getNotReadChatting = async (memberId) => {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/chats/api/chat/not-read/${memberId}`
    );
  ...
  const startChatting = async (memberId, profileMemberId) => {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/chats/api/chat-room/register?memberId=${memberId}&secondMemberId=${profileMemberId}`,
      {
        method: "POST",
      }
    );
  ...
  const getChatRoomList = async (memberId) => {
    let url = `${process.env.REACT_APP_BACKEND_URL}/chats/api/chat-room/list`;
    ...
    if (checkInputText === "") {
      ...
      url =
        url +
        `?memberId=${memberId}&filterType=${filterSearch}&searchNickname=${checkInputText}`;
    }
  ...
  const getChatList = async (memberId, chatRoomId) => {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/chats/api/chat/list?memberId=${memberId}&chatRoomId=${chatRoomId}`
    );
    const data = await response.json();
    setChatList(data);
  };
  ...
  const hideChatRoom = async (memberId, chatRoomId) => {
    await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/chats/api/chat-room/hide?chatRoomId=${chatRoomId}&memberId=${memberId}`,
      {
        method: "PUT",
      }
    );
  ...
  const hideChat = async (chatId, memberId) => {
    await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/chats/api/chat/hide?chatId=${chatId}&memberId=${memberId}`,
      {
        method: "PUT",
      }
    );
  ...
  const connect = (chatRoomList) => {
    stompClient.current = Stomp.over(
      () =>new WebSocket(`${process.env.REACT_APP_WEBSOCKET_URL}/ws`)
    );
    stompClient.current.connect({}, () => {
      chatRoomList.forEach((chatRoom) => {
        stompClient.current.subscribe(
          `/sub/chatroom/${chatRoom.chatRoomId}`,
          (message) => {
