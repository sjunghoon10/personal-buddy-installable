...
const MyPageAchievement = () => {
    ...
    setAchievementList((prev) => {
      return prev.map((item) => {
        ...
        if (item.id === achievementId) {
          ...
          const repCount = prev.filter(
            (a) => a.memberAchievementDisplay === 1
          ).length;
          // 이미 대표고 -> 해제는 허용
          if (isCurrentRep) {
  ...
  useEffect(() => {
    ...
    const getMemberAchievementList = async () => {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/achievements/api/achievement/completed/${memberId}`
      );
  ...
  useEffect(() => {
    ...
    const getMemberAchievementRep = async () => {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/achievements/api/achievement/displayed/${memberId}`
      );
      const datas = await response.json();
  ...
  const updateAchivementRep = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/achievements/api/achievement/change-display/${memberId}`,
      {
        method: "PUT",
        headers: {
      ...
      <S.MainContainer>
        ...
        <S.AchievementMainContainer>
          {achievementRep.length === 0 ? (
            <>
              <S.BuddyLogoImg
                src="/assets/images/logo/buddy-logo.png"
                alt="퍼스널 버디 로고 이미지"
              />
            ...
            achievementRep.map((item, i) => (
              ...
              >
                <S.AchievementIcon
                  ...
                  }/files/api/display?filePath=${
                    item.achievementImgPath
                  }&fileName=${encodeURIComponent(item.achievementImgName)}`}
                  alt="업적 이미지"
                  onError={(e) => {
                    e.target.src =
                      "/assets/images/header/default-achivement-img.png";
                  }}
                />
                ...
                <S.DescriptionListContainer>
                  <S.DescriptionContainer>
                    <img
                      src="/assets/images/contents/achievement/icons/achievement-check-ok.png"
                      alt="체크 아이콘"
                    />
                    <S.Description>
                      {item.achievementScheduleCategory} 일정
                    </S.Description>
                  </S.DescriptionContainer>
                  <S.DescriptionContainer>
                    <img
                    ...
                    />
                    <S.Description>
                      {item.achievementMissionCount}회 완료 시 달성
                    </S.Description>
                  </S.DescriptionContainer>
                  <S.DescriptionContainer>
                    <img
                    ...
                    />
                    <S.Description>
                      ({item.achievementCurrentMissionCount} /{" "}
                      {item.achievementMissionCount})
                    </S.Description>
                  </S.DescriptionContainer>
                  <S.DescriptionContainer>
                    <img
