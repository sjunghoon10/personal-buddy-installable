...
const MyPageProfileEdit = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      setProfileImage(
        `${process.env.REACT_APP_BACKEND_URL}/images/profile/${currentUser.memberImgName}`
      );
    }
  }, [currentUser, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      const profilePayload = {
        ...
        memberImgName:
          profileImage === defaultImg ? "" : currentUser.memberImgName,
        memberImgPath:
          profileImage === defaultImg ? "" : currentUser.memberImgPath,
      };
      ...
      if (data.newMemberImageInput) {
        ...
        const uploadRes = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/files/api/file-upload`,
          {
            method: "POST",
            body: imageForm,
          }
      ...
      const updateRes = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/members/api/update`,
        {
          method: "PUT",
          headers: {
  ...
  const handleNicknameBlur = async () => {
    ...
    try {
      const res = await fetch(
        ...
        }/members/api/nickname/check?nickname=${encodeURIComponent(nickname)}`,
        {
          method: "GET",
        }
      );
          ...
          <S.BodyContainer>
            <S.MemberProfile>
              ...
              <S.ImageMinusButton
                ...
                style={{
                  display:
                    profileImage !== "/assets/images/member/profile-default.png"
                      ? "block"
                      : "none",
                }}
              />
            ...
            <S.BottomContainer>
              <S.InputTextContainer>
                <S.InputTextTitle>
                  <h1>닉네임</h1>
                  <h2>
                    닉네임은 공백과 특수문자를 포함할 수 없고, 최대 12자 까지
                    입력할 수 있습니다.
                  </h2>
                </S.InputTextTitle>
                  ...
                  <S.MemberInfoInput
                    ...
                    {...register("memberNickName", {
                      ...
                      pattern: {
                        ...
                        message:
                          "공백, 특수 문자(!, @, # 등) 를 제외한 문자만 가능합니다. 최대 12자",
                      },
                    })}
                ...
                <S.ErrorMessage>
                  {errors.memberNickName && errors.memberNickName.message}
                </S.ErrorMessage>
              </S.InputTextContainer>
              <S.InputTextContainer>
                <S.InputTextTitle>
                ...
                <S.ErrorMessage>
                  {errors.memberComment && errors.memberComment.message}
                </S.ErrorMessage>
              </S.InputTextContainer>
              <S.SaveBtnContainer>
                <S.SaveBtn
