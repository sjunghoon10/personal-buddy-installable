...
const FindId = () => {
  ...
  const handleSendPhoneAuth = async () => {
    ...
    /*
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/sms/api/phone/send-code`, {
        method: 'POST',
        headers: {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/members/api/find/email`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <S.Wrapper>
        <form onSubmit={handleSubmit(onSubmit)}>
          <S.TitleWrapper>
            ...
            <S.StepText>
              <span>
                <S.ActiveStep>1. 본인확인 </S.ActiveStep> &gt; 2. 이메일 확인{" "}
              </span>
            </S.StepText>
          </S.TitleWrapper>
          <S.SubTitle>
          ...
          <S.Inputs>
            ...
            <S.InputWrapper>
              <img
                src="../assets/images/member/smile.png"
            ...
            <S.InputWrapper>
              <img
                src="../assets/images/member/phone-icon.png"
                className="input-icon"
                alt="전화"
              />
              <input
            ...
            {errors.phone && (
              <span style={{ color: "#FF3F3F" }}>{errors.phone.message}</span>
            )}
            ...
            {showAuthInput && (
              <S.InputWrapper>
                <img
                  src="../assets/images/member/lock-icon.png"
                  className="input-icon"
                  alt="인증"
                />
                <input
            ...
            {/* 메시지 */}
            {authResultMessage && (
              <div style={{ color: isAuthValid ? "#01CD74" : "#FF3F3F" }}>
                {authResultMessage}
              </div>
            )}
            ...
            <S.SignupButton
              type="submit"
              className={
                isAuthValid && watch("memberName") && watch("phone")
                  ? "active"
                  : ""
              }
            >
